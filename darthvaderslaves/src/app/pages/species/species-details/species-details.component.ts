...
export class SpeciesDetailsComponent {
  ...
  ngOnInit(): void {
    this.attachRouterId();
    this.swapiService.getDataById<Species>(this.id, 'species').subscribe((data)=>{
      this.species = data;
    })
  ...
  getImagePath(): string {
    return `assets/species/${this.id}.jpg`;
  }
  ...
  navigateToSpecies(){
    ...
    this.router.navigate(['/species'], { queryParams: { page: pageNumber } })
  }
